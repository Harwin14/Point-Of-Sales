    ...
    router.post('/edit/:barcode', isLoggedIn, async (req, res, next) => {
        try {
            ...
            if (!req.files || Object.keys(req.files).length === 0) {
                ...
                await db.query('UPDATE goods SET name=$1, stock=$2, purchaseprice=$3, sellingprice=$4, unit=$5, picture=$6 WHERE barcode=$7', [name, stock, purchase, selling, unit, imagesfiles, barcode])
            }
            res.redirect('/goods')
        } catch (err) {
            res.send(err)
